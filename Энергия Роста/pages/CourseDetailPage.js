...
const CourseDetailPage = ({ lang, t, progress, course, lessons, onSelectLesson, navigate }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 animate-fade-in">
      <button onClick={() => navigate('courses')} className="text-sm text-gray-500 hover:text-primary-600 mb-4 inline-block">← {t('backToCourse')}</button>
      ...
      <div className="space-y-3">
        {lessons.map((lesson, idx) => {
          ...
          return (
            <Card key={lesson.id} onClick={() => onSelectLesson(lesson)} className="flex items-center gap-4">
